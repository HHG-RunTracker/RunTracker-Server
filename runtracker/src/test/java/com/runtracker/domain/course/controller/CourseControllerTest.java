...
class CourseControllerTest extends RunTrackerDocumentApiTester {
    ...
    @Test
    void getRecommendedCoursesBySettingTest() throws Exception {
        // given
        List<CourseDetailDTO> mockRecommendedCourses = List.of(
                CourseDetailDTO.builder()
                        .id(1L)
        ...
        given(courseService.getRecommendedCoursesBySetting(anyLong(), any(Double.class), any(Double.class)))
                .willReturn(mockRecommendedCourses);
        given(jwtUtil.getMemberIdFromToken(anyString())).willReturn(1L);
        given(jwtUtil.getSocialIdFromToken(anyString())).willReturn("kakao_123");
        ...
        // when
        this.mockMvc.perform(get("/api/courses/recommend/setting")
                        .header(AUTH_HEADER, TEST_ACCESS_TOKEN)
                        .param("latitude", "37.5665")
                        .param("longitude", "126.9780"))
                .andExpect(status().isOk())
                .andDo(document("course-get-recommendations-setting",
                        resource(
                                ResourceSnippetParameters.builder()
                                        .tag("courses")
                                        .summary("AI 설정 기반 코스 추천")
                                        .description("사용자 런닝 설정(일간 거리 목표, 선호 난이도)과 주변 코스 기반으로 AI 코스 추천")
                                        .requestHeaders(
                                                headerWithName("Authorization").description("엑세스 토큰")
                                        )
                                        .queryParameters(
